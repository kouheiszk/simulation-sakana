...
int seika(int tane) {
	int i, j, randam = 0;
	double t, I1, I2, HAKKA1, HAKKA2;

	int N1[3] = {1}, N2[3] = {1}, count[3] = {0};
	bool hakka1[3] = {0},  hakka2[3] = {0};
	
	double I[3] = {0.0}, V[3] = {0.0};
	double w1[3] = {1.0}; // シナプスの結合強度
	double Itime[3][max];
	double I1time[3] = {0.0};
	double I2time[3] = {0.0};
	double HAKKAtime[3][max] = {0.0};
	...
	for(t = 0.000; t <= tmax + h; t += h) {
		...
		for(i = 0; i < 3; i++) {
			...
			HAKKA1 = keisanHakka(t, count[(i + 1) % 3], HAKKAtime[(i + 1) % 3]);
			HAKKA2 = keisanHakka(t, count[(i + 2) % 3], HAKKAtime[(i + 2) % 3]);
			I[i] = w1[i] * I1 + w2 * I2 + 0.75 * HAKKA1 + 0.75 * HAKKA2; // 入力電流の合計
