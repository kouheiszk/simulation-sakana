...
#define DELAY               0.007 // PreとPostのスパイクの位相差[ms]
#define SEED                6 // srand関数の初期化のための数字
...
double func(double V, double W1, double t, double pre_spike_time[], double post_spike_time[]) {
	...
	for(i = 0; post_spike_time[i] <= t; i++) {
		V += W1 * alpha(t, post_spike_time[i]);
	}
		...
		for(unit = 0; unit < UNIT_MAX; unit++) {
			...
			for(unit_interaction = 0; unit_interaction < UNIT_MAX; unit_interaction++) {
				...
				if(t >= hakka_time[unit][hakka_count[unit] - 1] || t >= hakka_time[unit_interaction][hakka_count[unit_interaction] - 1]) {
					dt = hakka_time[unit][hakka_count[unit] - 1] - hakka_time[unit_interaction][hakka_count[unit_interaction] - 1];
					if(dt > 0) {
						WIJ[unit][unit_interaction] = STDP(WIJ[unit][unit_interaction], DT, 0);
					} else {
						WIJ[unit][unit_interaction] = STDP(WIJ[unit][unit_interaction], 0, DT);
					}
				}
			}
