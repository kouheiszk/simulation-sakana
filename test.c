...
//ここでI1を計算
double keisanI1(double t, int M) {
int i;
double I = 0.0;
double ti = 0.0;
double derutaW = 0.0;
...
double keisanHakka(double t, int N[], double HAKKAtime[][max],int j) { 
int k=0;
double I = 0.0;
if(N[j] >= 0) {
for(k = 0; k <= N[j]; k++) {
if(t >= HAKKAtime[j][k]) {
I = I + I0 * (((t - HAKKAtime[j][k]) / tauarufa) * exp(-(t - HAKKAtime[j][k]) / tauarufa));
}
}
}
return I;
