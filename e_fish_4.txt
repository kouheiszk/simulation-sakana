...
#define UNIT_MAX            3 // ニューロンの最大数
#define DELAY               0.005 // PreとPostのスパイクの位相差[ms]
#define SEED_START          1 // srand関数の初期化のための数字
#define SEED_END            1 // srand関数の初期化のための数字

#define UNIT_MIDDLE_MAX     1 // 中央のニューロンの数
...
#define W_NETWORK           0.75 // ネットワークの結合強度
#define W_MIDDLE            0.915 // 中央のニューロンへの結合強度
...
double middle_func(double V, double W, double t, double hakka_time[UNIT_MAX][HAKKA_MAX], int hakka_count[]) {
	int i, unit;
	...
	for(unit = 0; unit < UNIT_MAX; unit++) {
		// 発火していなければ足さない
		if(hakka_count[unit] == 0) continue;
		for(i = 0; hakka_time[unit][i] <= t && i < hakka_count[unit]; i++) {
			V += W * alpha(t, hakka_time[unit][i]);
		}
	}
...
double MIDDLE_LIF(double V, double W, double t, double hakka_time[UNIT_MAX][HAKKA_MAX], int hakka_count[]) {
	double k1, k2, k3, k4;

	k1 = DT * middle_func(V,            W, t,            hakka_time, hakka_count);
	k2 = DT * middle_func(V + k1 * 0.5, W, t + DT * 0.5, hakka_time, hakka_count);
	k3 = DT * middle_func(V + k2 * 0.5, W, t + DT * 0.5, hakka_time, hakka_count);
	k4 = DT * middle_func(V + k3,       W, t + DT,       hakka_time, hakka_count);
...
void calc(int seed) {
	...
	double V[UNIT_MAX] = {V_RESET}; // ニューロンの電位
	...
	double V_MIDDLE[UNIT_MIDDLE_MAX] = {V_RESET};
	int middle_hakka_count[UNIT_MIDDLE_MAX] = {0}; // ニューロンの発火回数
	double middle_hakka_time[UNIT_MIDDLE_MAX][HAKKA_MAX] = {0}; // 発火した時刻
	...
	for(t = 0; t < T_MAX; t += DT) {
		for(unit = 0; unit < UNIT_MAX; unit++) {
			...
			if(V[unit] < V_THETA) {
				if(UNIT_MAX > 1) {
					V[unit] = NETWORK_LIF(V[unit], W1[unit], t, pre_spike_time[unit], post_spike_time[unit], unit, hakka_time, hakka_count);
				} else {
		...
		// 中央の集約ニューロンに発火を伝えるというか、伝わる
		if(UNIT_MIDDLE_MAX > 0) {
			for(unit_middle = 0; unit_middle < UNIT_MIDDLE_MAX; unit_middle++) {
				// 中央の集約ニューロンの電位が閾値を超えていたら
				if(V_MIDDLE[unit_middle] < V_THETA) {
					V_MIDDLE[unit_middle] = MIDDLE_LIF(V_MIDDLE[unit_middle], W_MIDDLE, t, hakka_time, hakka_count);

					if(V_MIDDLE[unit_middle] >= V_THETA) {
						V_MIDDLE[unit_middle] = V_MAX;
						// 発火時刻を記録する
						middle_hakka_time[unit_middle][middle_hakka_count[unit_middle]] = t;
						middle_hakka_count[unit_middle]++;

						// ニューロンの発火時刻と発火回数を表示する
						printf("Time: %9.3lf, Unit: %2d, %3d times\n", t, unit_middle + 1, middle_hakka_count[unit_middle]);
					}
				} else if(V_MIDDLE[unit_middle] >= V_THETA) {
					V_MIDDLE[unit_middle] = V_RESET;
				}
			}
		}
	}
	...
	printf("----------\n");
	// 結果の出力（位相差と乱数の種）
	printf("Unit Middle: %2d\n", UNIT_MIDDLE_MAX);
	// 結果の出力（発火回数）
	for(unit_middle = 0; unit_middle < UNIT_MIDDLE_MAX; unit_middle++) {
		printf("Unit Middle%2d: %4d times\n", unit_middle + 1, middle_hakka_count[unit_middle]);
	}
	printf("--------------------\n");
}
