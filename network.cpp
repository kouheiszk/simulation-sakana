...
void main()
{
	fp1 = fopen("stdp1_0_0.txt","w+");//ファイルオープン
	if(fp1 == NULL){
		exit(1);
	}

	fp2 = fopen("spike1_0_0.txt","w+");//ファイルオープン
	if(fp2 == NULL){
		exit(1);
	}

	fp3 = fopen("spike2_0_0.txt","w+");//ファイルオープン
	if(fp3 == NULL){
		exit(1);
	}

	fp4 = fopen("output1_0_0.txt","w+");//ファイルオープン
	if(fp4 == NULL){
		exit(1);
	}

	fp5 = fopen("raster1_0_0.txt","w+");//ファイルオープン
	if(fp5 == NULL){
		exit(1);
	}

	fp6 = fopen("stdp2_0_0.txt","w+");//ファイルオープン
	if(fp6 == NULL){
		exit(1);
	}

	fp7 = fopen("output2_0_0.txt","w+");//ファイルオープン
	if(fp7 == NULL){
		exit(1);
	}

	fp8 = fopen("raster2_0_0.txt","w+");//ファイルオープン
	if(fp8 == NULL){
		exit(1);
	}

	fp9 = fopen("stdp3_0_0.txt","w+");//ファイルオープン
	if(fp9 == NULL){
		exit(1);
	}

	fp10 = fopen("output3_0_0.txt","w+");//ファイルオープン
	if(fp10 == NULL){
		exit(1);
	}

	fp11 = fopen("raster3_0_0.txt","w+");//ファイルオープン
	if(fp11 == NULL){
		exit(1);
	}

	fp12 = fopen("count_0_0.txt","w+");//ファイルオープン
	if(fp12 == NULL){
		exit(1);
	}
	fp13 = fopen("raster'1_0_0.txt","w+");//ファイルオープン
	if(fp13 == NULL){
		exit(1);
	}

	fp14 = fopen("raster'2_0_0.txt","w+");//ファイルオープン
	if(fp14 == NULL){
		exit(1);
	}

	fp15 = fopen("raster'3_0_0.txt","w+");//ファイルオープン
	if(fp15 == NULL){
		exit(1);
	}
